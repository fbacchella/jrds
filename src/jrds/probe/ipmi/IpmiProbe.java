        ...
        while(value.isNaN()) {
            try {
                ...
                try {
                    ...
                    log(Level.DEBUG, "read %f for %s", data2.getSensorReading(fsr), getLabel());
                    value = data2.getSensorReading(fsr);
                } catch (IPMIException e) {
                    if (e.getCompletionCode() == CompletionCode.DataNotPresent) {
                        log(Level.WARN, "%s", e.getMessage());
                    } else {
                        throw e;
                    }
                }
                ...
                if(e.getCompletionCode() == CompletionCode.ReservationCanceled) {
                    ...
                    // appears.
                    try {
                        reservationId = ((ReserveSdrRepositoryResponseData) handle
                                .sendMessage(new ReserveSdrRepository(IpmiVersion.V20, handle.getCipherSuite(),
